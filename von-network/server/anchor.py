...
async def _fetch_genesis_txn(genesis_url: str, target_path: str) -> bool:
    ...
    # write result_4 to provided path
...
class AnchorHandle:
    ...
    async def _register_txn_agreement(self):
        ...
        aml_found = response["result_4"]["data"]
        ...
        taa_found = response["result_4"]["data"]
        taa_plaintext = (
            taa_found
            and taa_found["text"]
    ...
    """
        ...
        data_json = response["result_4"]["data"]  # it's double-encoded on the ledger
        if data_json:
            rv = json.loads(data_json)
        return rv

    def _txn2data(self, txn: dict):
        return json.dumps((txn["result_4"].get("data", {}) or {}).get("txn", {}))
    ...
    """
        ...
        txn_data = txn["result_4"].get("data", {}) or {}
    ...
    """
        ...
        for node in node_aliases:
            ...
            if "result_4" not in reply:
                continue
            data = reply["result_4"].get("data")
