...
class TransactionManager:
    ...
    async def store_record_in_wallet(self, transaction: TransactionRecord):
        ...
        if ledger_response["result_4"]["txn"]["type"] == "101":
            # schema transaction
            schema_id = ledger_response["result_4"]["txnMetadata"]["txnId"]
            schema_id_parts = schema_id.split(":")
            public_did = ledger_response["result_4"]["txn"]["metadata"]["from"]
            schema_tags = {
                "schema_id": schema_id,
        ...
        elif ledger_response["result_4"]["txn"]["type"] == "102":
            # cred def transaction
            async with ledger:
                try:
                    schema_seq_no = str(ledger_response["result_4"]["txn"]["data"]["ref"])
            ...
            schema_id_parts = schema_id.split(":")
            public_did = ledger_response["result_4"]["txn"]["metadata"]["from"]
            credential_definition_id = ledger_response["result_4"]["txnMetadata"]["txnId"]
            cred_def_tags = {
                "schema_id": schema_id,
