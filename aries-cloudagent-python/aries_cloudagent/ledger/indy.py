...
class IndySdkLedger(BaseLedger):
        ...
        if schema_info:
            ...
            try:
                ...
                try:
                    ...
                    seq_no = json.loads(resp)["result_4"]["txnMetadata"]["seqNo"]
    ...
    async def fetch_schema_by_id(self, schema_id: str) -> dict:
        ...
        if not response["result_4"]["seqNo"]:
            # schema not found
            return None
        ...
        if parsed_response and self.pool.cache:
            await self.pool.cache.set(
                [f"schema::{schema_id}", f"schema::{response['result_4']['seqNo']}"],
                parsed_response,
    ...
    async def fetch_schema_by_seq_no(self, seq_no: int):
        ...
        data_txn = (response["result_4"].get("data", {}) or {}).get("txn", {})
    ...
    async def get_key_for_did(self, did: str) -> str:
        ...
        response_json = await self._submit(request_json, sign_did=public_info)
        data_json = (json.loads(response_json))["result_4"]["data"]
    ...
    async def get_nym_role(self, did: str) -> Role:
        ...
        response = json.loads(response_json)
        nym_data = json.loads(response["result_4"]["data"])
        if not nym_data:
    ...
    async def rotate_public_did_keypair(self, next_seed: str = None) -> None:
        ...
        response_json = await self._submit(request_json)
        data = json.loads((json.loads(response_json))["result_4"]["data"])
        if not data:
            raise BadLedgerRequestError(
        ...
        txn_resp_data = txn_resp["result_4"]["data"]
    ...
    async def fetch_txn_author_agreement(self) -> dict:
        ...
        aml_found = (json.loads(response_json))["result_4"]["data"]
        ...
        taa_found = (json.loads(response_json))["result_4"]["data"]
    ...
    async def get_revoc_reg_def(self, revoc_reg_id: str) -> dict:
        ...
        try:
            ...
            found_def["txnTime"] = json.loads(response_json)["result_4"]["txnTime"]
